const { useState, useEffect, useRef, useMemo } = React;
        ...
        // --- SUB-COMPONENT: MEDIA UPLOADS ---
        function MediaUploads({ formData, setFormData, updatePref, connectionStatus, onBack, onWhatsAppShare }) {
            ...
            const handlePhotoFile = (file) => {
                ...
                reader.onload = (e) => {
                    setFormData(prev => ({ ...prev, photoBase64: e.target.result }));
                };
                ...
                <div className="tab-content active">
                        ...
                        <div className="form-group">
                            ...
                            >
                                ...
                                {formData.photoBase64 && (
                                    <img className="preview-thumb" src={formData.photoBase64} alt="Profile Preview" />
                                )}
                            </div>
                            {formData.photoBase64 && (
                    ...
                    <div className="setup-grid">
                        <div className="form-group" style={{ gridColumn: 'span 2' }}>
                            <label style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '0.5rem', width: '100%' }}>
                                <span>Google Sheet Web App URL <span className="optional">(For automatic sync)</span></span>
                                <span className={`status-badge ${connectionStatus ? 'badge-connected' : 'badge-offline'}`}>
                                    {connectionStatus ? 'Connected' : 'Local Vault Only'}
                                </span>
                            </label>
                            <div className="input-wrapper">
                                <svg fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                ...
                                <input 
                                    ...
                                    onChange={(e) => updatePref('googleSheetsUrl', e.target.value)}
                                />
                            </div>
                            <span className="canvas-info">Leave blank to save records locally inside the Vault first, or paste your web app macro to write directly to Google Sheets!</span>
                        </div>

                        <div className="form-group">
                            <label>Admin WhatsApp Number <span className="optional">(For direct text dispatch)</span></label>
                            <div className="input-wrapper">
                                <svg fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <div className="form-actions">
                        ...
                        <button type="button" className="btn btn-secondary" style={{ borderColor: '#25d366', color: '#25d366', background: 'rgba(37, 211, 102, 0.05)' }} onClick={onWhatsAppShare}>
        ...
        function VaultDrawer({ vault, onLoad, onShare, onDelete, onClear }) {
            return (
                <div className="tab-content active">
                    ...
                    {vault.length === 0 ? (
                        <div className="vault-empty" style={{ display: 'flex' }}>
                            <svg fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        ...
                        <div className="vault-grid" style={{ display: 'grid' }}>
        ...
        function CardPreview({ formData, isFlipped, cardFrontRef, cardBackRef }) {
            ...
            return (
                <div className={`card-scene ${isFlipped ? 'flipped' : ''}`} ref={sceneRef}>
                    <div className="card-viewport" ref={viewportRef}>
                        
                        {/* ID CARD FRONT */}
                        <div className="id-card id-card-front" ref={cardFrontRef}>
                            <div className="card-holo" ref={holoFrontRef}></div>
                            ...
                            <div className="card-body">
                                <div className="card-avatar-container">
                                    {!formData.photoBase64 ? (
                                        ...
                                        <img className="card-photo" src={formData.photoBase64} alt="Profile" style={{ display: 'block' }} />
                                    )}
                                </div>

                                <div className="card-field-group">
                                    <div className="card-name">{formData.fullName || 'EMPLOYEE NAME'}</div>
                                </div>
                                <div className="card-field-group">
                                    <div className="card-title">{(formData.designation || 'DESIGNATION').toUpperCase()}</div>
                                </div>

                                <div className="card-info-table">
                                    <div className="card-info-row">
                                        <span className="card-info-label">Department</span>
                                        <span className="card-info-val">{formData.department || 'Department'}</span>
                                    </div>
                                    <div className="card-info-row">
                                        <span className="card-info-label">Blood Group</span>
                                        <span className="card-badge">{formData.bloodGroup || '-'}</span>
                                    </div>
                                </div>
                            </div>
                        ...
                        {/* ID CARD BACK */}
                        <div className="id-card id-card-back" ref={cardBackRef}>
                            <div className="card-holo" ref={holoBackRef}></div>
                            <div className="card-slot"></div>
                            ...
                            <div className="card-back-body">
                                ...
                                <div className="card-back-info">
                                    <div className="card-back-row">
                                        ...
                                        <span className="card-back-val">{formData.mobileNo || '-'}</span>
                                    </div>
                                    <div className="card-back-row">
                                        <span className="card-back-label">Emergency Call</span>
                                        <span className="card-back-val">{formData.emergencyContact || '-'}</span>
                                    </div>
                                    <div className="card-back-row">
                                    ...
                                    <div className="card-back-row" style={{ marginTop: '2px' }}>
                                        ...
                                        <span className="card-back-val" style={{ color: 'var(--card-text-primary, #ffffff)', fontWeight: 600 }}>{formattedValidity}</span>
                                    </div>
                                </div>
                                ...
                                <div className="card-bottom-row">
                                    <div className="card-signature-wrapper">
                                        <div className="card-signature-box">
                                            {!formData.signatureBase64 ? (
                                                <span className="card-signature-placeholder">NO SIGNATURE</span>
                                            ) : (
                                                <img className="card-signature-img" src={formData.signatureBase64} alt="Authorized Signature" style={{ display: 'block' }} />
                                            )}
                                        </div>
                                        <span className="card-signature-title">Card Holder</span>
                                    </div>
                                    
                                    <div className="card-qrcode-box" title="Dynamic Card Verification Code">
        ...
        function App() {
            ...
            useEffect(() => {
                ...
                const cachedVault = JSON.parse(localStorage.getItem('hox_card_vault') || '[]');
            ...
            const saveToVault = (payload) => {
                let currentVault = vault.length > 0 ? [...vault] : JSON.parse(localStorage.getItem('hox_card_vault') || '[]');
                const existingIndex = currentVault.findIndex(item => item.fullName === payload.fullName);
                
                if (existingIndex > -1) {
                    currentVault[existingIndex] = payload;
                } else {
                    currentVault.unshift(payload);
                }

                try {
                    localStorage.setItem('hox_card_vault', JSON.stringify(currentVault));
                } catch (e) {
                    console.warn("Vault storage quota exceeded, item will only exist in active session:", e);
                    triggerToast('Vault Storage Full!', 'Limit exceeded. Delete old cards or clear the vault to save new ones permanently.', 'error', 8000);
                }
                setVault(currentVault);
            };

            const loadFromVault = (record) => {
                setFormData(prev => ({
                    ...prev,
                    ...record
                }));
                triggerToast('Card Loaded!', `Form fields populated with details of ${record.fullName}.`, 'success', 3000);
                setActiveTab('form-section');
                setIsFlipped(false);
            };

            const deleteFromVault = (index, name) => {
                if (!confirm(`Are you sure you want to delete ${name}'s card from your local Vault?`)) return;
                let currentVault = [...vault];
                currentVault.splice(index, 1);
                localStorage.setItem('hox_card_vault', JSON.stringify(currentVault));
                setVault(currentVault);
                triggerToast('Record Deleted', 'Card successfully deleted from history.', 'error', 2500);
            };

            const clearVault = () => {
                if (!confirm('Warning: This will clear ALL saved ID card history from this computer. Continue?')) return;
                localStorage.removeItem('hox_card_vault');
                setVault([]);
                triggerToast('Vault Cleared', 'All local submissions successfully deleted.', 'error', 3000);
            };
...
*Submitted via Hox ID Builder Portal*`;
            ...
            const handleSubmit = async (e) => {
                ...
                if (formData.googleSheetsUrl) {
                    ...
                    try {
                        ...
                        triggerToast('Network Error', 'Failed to reach central server. Record saved locally in Vault.', 'error', 6000);
                    }
                } else {
                    triggerToast('Saved Locally!', 'Database configuration not found. Card saved inside local Vault.', 'warning', 5000);
                }
            ...
            const exportHighResPrints = async () => {
                ...
                showLoader('Preparing Printable Snapshots...', 'Generating front & back card prints. Please hold...');
